...
const setup = deployments.createFixture(async () => {
  ...
  const mockMessenger: MockMessenger = await ethers.getContract(
    CONTRACT_NAMES.MockMessenger
  );
  ...
  const mockMessengerBis: MockMessenger = await ethers.getContractAt(
    CONTRACT_NAMES.MockMessenger,
    tx_deploy.address
  );
  ...
  let tx_wl = await slaRegistry.createSLA(
    ...
    'dummy-ipfs-hash',
    activeWlSLAConfig.severity,
    activeWlSLAConfig.penalty,
    activeWlSLAConfig.leverage
  );

  await tx_wl.wait();
  const awlSlaAddress = (await slaRegistry.allSLAs()).slice(-1)[0];
  const sla_wl: SLA = await ethers.getContractAt(
    CONTRACT_NAMES.SLA,
    awlSlaAddress
  );
  await tx_wl.wait();
...
describe(CONTRACT_NAMES.Staking, function () {
  ...
  describe('staking', function () {
    it('should perform staking long position for deployer', async () => {
      ...
      let detailsarrs = await details.getSLADetailsArrays(sla.address);
    ...
    it('should perform staking short position for deployer when user have long position in pool', async () => {
      ...
      await sla
        .connect(user)
        .stakeTokens(amount * leverage, dslaToken.address, POSITION.OK);

      await dslaToken.connect(owner).approve(sla.address, amount * leverage);
      await expect(
        sla
          .connect(owner)
          .stakeTokens(amount * leverage, dslaToken.address, POSITION.KO)
      )
        .to.emit(sla, 'Stake')
        .withArgs(
          dslaToken.address,
    ...
    it('should prevent staking short position for deployer if insufficient long position present in staking pool', async () => {
      ...
      await expect(
        sla.stakeTokens(stakeAmount, dslaToken.address, POSITION.KO)
    ...
    it('should prevent staking in case of amount exceeds allowance', async () => {
      ...
      await expect(
        sla.stakeTokens(stakeAmount, dslaToken.address, POSITION.OK)
    ...
    it('should prevent staking in case of invalid token address', async () => {
      ...
      await expect(
        sla.stakeTokens(stakeAmount, invalidTokenAddress, POSITION.OK)
      ).to.be.reverted;
    });
    ...
    it('should prevent staking in case of invalid position side', async () => {
      ...
      await expect(
        sla.stakeTokens(stakeAmount, dslaToken.address, invalidPositionSide)
      ).to.be.reverted;
    });
  });
  describe('add allowed tokens - Creating DTokens', function () {
    it('should add allowed tokens in list if not yet present then perform staking', async () => {
      ...
      expect(sla.addAllowedTokens(dslaToken.address)).to.emit(
        sla,
        'DTokensCreated'
      );
    });
    ...
    it('should revert if token already present in allowedTokens', async () => {
      const { sla, dslaToken } = fixture;
      await sla.addAllowedTokens(dslaToken.address);
      await expect(sla.addAllowedTokens(dslaToken.address)).to.be.revertedWith(
    ...
    it('should revert if token is not allowed in StakeRegistry', async () => {
      const { sla } = fixture;
      await expect(
        sla.addAllowedTokens(ethers.constants.AddressZero)
      ).to.be.revertedWith('This token is not allowed.');
    });
    ...
    it('should revert if it exceeds max token length in StakeRegistry', async () => {
      ...
      await expect(sla.addAllowedTokens(token1.address)).to.be.revertedWith(
        'max token length'
      );
    });
  });
  describe('whitelist', function () {
    ...
    it('should perform staking for contract owner even if whitelist is activated', async () => {
      ...
      await expect(
        sla_wl.stakeTokens(stakeAmount, dslaToken.address, POSITION.OK)
      )
      ...
      let detailsarrs = await details.getSLADetailsArrays(sla_wl.address);
    ...
    it('should prevent staking for not whitelisted user if whitelist is activated', async () => {
      ...
      await expect(
        sla_wl
          .connect(user)
          .stakeTokens(stakeAmount, dslaToken.address, POSITION.OK)
