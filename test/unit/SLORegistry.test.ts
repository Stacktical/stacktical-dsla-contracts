...
const getDeviation = (
  ...
  return Math.floor(
    (Math.abs(sliValue - sloValue) * precision) / ((sloValue + sliValue) / 2)
  );
};
...
describe(CONTRACT_NAMES.SLORegistry, function () {
  ...
  it('SLO registration is only allowed for SLARegistry', async function () {
    ...
    await expect(
      deployerSLO.registerSLO(0, 0, ethers.constants.AddressZero)
    ).to.be.revertedWith(
  ...
  describe('checks sli value against SLOValue - isRespected', () => {
    const config = {
      ...
      severity: baseSLAConfig.severity,
      penalty: baseSLAConfig.penalty,
    };
    it('GreaterThan', async () => {
      ...
      expect(await sloRegistry.isRespected(baseSLAConfig.sloValue, slaAddress))
        .to.be.false;
      expect(
        await sloRegistry.isRespected(baseSLAConfig.sloValue + 1, slaAddress)
      ).to.be.true;
      expect(
        await sloRegistry.isRespected(baseSLAConfig.sloValue - 1, slaAddress)
      ).to.be.false;
  ...
  describe('checks deviations - getDeviation', () => {
    ...
    it('GreaterThan', async () => {
      ...
      expect(deviation).to.be.equal(
        BigNumber.from(getDeviation(sloValue, sliValue, precision))
      );
      expect(
        await sloRegistry.getDeviation(
          sliValue / 2,
          slaAddress,
          config.severity,
          config.penalty
