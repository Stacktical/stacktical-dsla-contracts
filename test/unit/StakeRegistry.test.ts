...
const deployMessenger = async (
  ...
  const mockMessenger: MockMessenger = await ethers.getContractAt(
...
const deploySLA = async (slaConfig: SLAConfig) => {
  ...
  const mockMessenger = await deployMessenger(
    deployer,
    periodRegistry.address,
    stakeRegistry.address
  );
...
describe(CONTRACT_NAMES.StakeRegistry, function () {
  ...
  describe('constructor', function () {
    it('should register dsla token at constructor', async () => {
      ...
      expect(await stakeRegistry.DSLATokenAddress()).to.be.equal(
        dslaToken.address
      );
      expect(await stakeRegistry.allowedTokens(0)).to.be.equal(
        dslaToken.address
      );
    });
    it('should revert constructor if DSLA token address is invalid', async () => {
      ...
      await expect(
        deploy(CONTRACT_NAMES.StakeRegistry, {
          from: owner.address,
          log: true,
  ...
  describe('token allowance', function () {
    it('should allow only owner to add allowed tokens', async () => {
      ...
      await expect(
        stakeRegistry.addAllowedTokens(dslaToken.address)
      ...
      await expect(
        stakeRegistry.connect(user).addAllowedTokens(dslaToken.address)
  ...
  it('should allow registration of owners of staking only for registered slas', async () => {
    ...
    await expect(
      stakeRegistry.registerStakedSla(owner.address)
    ...
    expect(await stakeRegistry.slaWasStakedByUser(owner.address, slaAddress)).to
      .be.true;
    expect(await stakeRegistry.slaWasStakedByUser(owner.address, owner.address))
      .to.be.false;
  });
  describe('creating dToken', function () {
    ...
    it('should create dTokens when staking tokens on sla', async () => {
      ...
      await deploySLA(baseSLAConfig);
      const slaAddress = (await slaRegistry.allSLAs()).slice(-1)[0];
      const sla: SLA = await ethers.getContractAt(
        CONTRACT_NAMES.SLA,
        slaAddress
      );

      await expect(sla.addAllowedTokens(dslaToken.address)).to.emit(
        stakeRegistry,
        'DTokenCreated'
      );
    });
  ...
  it('should create dTokens with same decimals as registered token', async () => {
    ...
    const duToken: DToken = await ethers.getContractAt(
      CONTRACT_NAMES.dToken,
      duTokenAddr
    );
  ...
  describe('Rewards Distribution', function () {
    it('should distribute verification rewards when requesting sli on slaRegistry', async () => {
      ...
      await expect(
        slaRegistry.requestSLI(Number(nextVerifiablePeriod), slaAddress, false)
    ...
    it('should recieve verfication rewards', async () => {
      ...
      expect(await dslaToken.balanceOf(owner.address)).to.be.eq(
        BigNumber.from(10).pow(18).mul(750).add(dslaBalance)
      );
    });
    it('should verify period when distributing rewards', async () => {
      ...
      expect(
        await stakeRegistry.periodIsVerified(slaAddress, nextVerifiablePeriod)
      ).to.be.true;
    });
  ...
  describe('Return locked value', () => {
    it('should allow returnLockedValue only for SLARegistry', async () => {
      ...
      await expect(
        stakeRegistry
          .connect(owner)
          .returnLockedValue(Ethers.constants.AddressZero)
      ).to.be.revertedWith('Can only be called by SLARegistry');
    });
  });
  describe('Utils', function () {
    it('should allow only owner to set staking parameters', async () => {
      const { stakeRegistry } = fixture;
      await expect(
        stakeRegistry.setStakingParameters(
